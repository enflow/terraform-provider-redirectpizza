---
page_title: "redirect.pizza Provider"
description: |-
  The redirect.pizza provider manages redirects with the redirectpizza_redirect resource.
---

# redirect.pizza Provider

Terraform provider for [redirect.pizza](https://redirect.pizza). Use the `redirectpizza_redirect` resource to manage redirects as code.

Install it from the Terraform Registry as `enflow/redirectpizza`.

Use this provider together with your DNS provider. You may hardcode values from the [DNS records](https://redirect.pizza/support/dns-type) documentation, or your own [Dedicated IP](https://redirect.pizza/support/dedicated-ip).



## Example Usage

```terraform
terraform {
  required_providers {
    redirectpizza = {
      source  = "enflow/redirectpizza"
      version = "~> 0.2"
    }
  }
}

# Set the variable value in a *.tfvars file or with -var="rp_token=..."
variable "rp_token" {
  type      = string
  sensitive = true
}

provider "redirectpizza" {
  token = var.rp_token
}
```



<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `token` (String, Sensitive) API token from the redirect.pizza dashboard (More → API).

### Optional

- `api_base_url` (String) Base URL for the redirect.pizza API.
